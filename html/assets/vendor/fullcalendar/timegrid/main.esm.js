...
import {  } from '@fullcalendar/core';
import { DayBgRowDayGridSimpleDayGrid } from '@fullcalendar/daygrid';
...
var TimeGridEventRenderer = /** @class */ (function (_super) {
    ...
    TimeGridEventRenderer.prototype.assignSegCss = function (segs) {
        for (var _i = 0segs_1 = segs; _i < segs_1.length; _i++) {
            var seg = segs_1[_i];
            applyStyle(seg.elthis.generateSegCss(seg));
            if (seg.level > 0) {
...
var TimeGrid = /** @class */ (function (_super) {
    ...
    function TimeGrid(contextelrenderProps) {
        ...
        var renderColumns = _this.renderColumns = memoizeRendering(_this._renderColumns_this._unrenderColumns);
        ...
        _this.renderDateSelection = memoizeRendering(_this._renderDateSelection_this._unrenderDateSelection[renderColumns]);
        ...
        _this.renderEventDrag = memoizeRendering(_this._renderEventDrag_this._unrenderEventDrag[renderColumns]);
        _this.renderEventResize = memoizeRendering(_this._renderEventResize_this._unrenderEventResize[renderColumns]);
    ...
    TimeGrid.prototype.processOptions = function () {
        ...
        snapsPerSlot = wholeDivideDurations(slotDurationsnapDuration);
    ...
    TimeGrid.prototype.computeLabelInterval = function (slotDuration) {
        ...
        for (i = AGENDA_STOCK_SUB_DURATIONS.length - 1; i >= 0; i--) {
            ...
            slotsPerLabel = wholeDivideDurations(labelIntervalslotDuration);
    ...
    TimeGrid.prototype._renderSlats = function (dateProfile) {
        ...
        this.slatPositions = new PositionCache(this.elthis.slatElsfalsetrue // vertical
        );
    ...
    TimeGrid.prototype.renderSlatRowHtml = function (dateProfile) {
        ...
        while (asRoughMs(slotTime) < asRoughMs(dateProfile.maxTime)) {
            ...
            isLabeled = wholeDivideDurations(slotIteratorthis.labelInterval) !== null;
            axisHtml =
                '<td class="fc-axis fc-time ' + theme.getClass('widgetContent') + '">' +
                    (isLabeled ?
                        '<span>' + // for matchCellWidths
                            htmlEscape(dateEnv.format(slotDatethis.labelFormat)) +
                            '</span>' :
                        '') +
                    '</td>';
            html +=
                '<tr data-time="' + formatIsoTimeString(slotDate) + '"' +
                    ...
                    (isRtl ? axisHtml : '') +
                    '</tr>';
            slotTime = addDurations(slotTimethis.slotDuration);
            slotIterator = addDurations(slotIteratorthis.slotDuration);
        }
        return html;
    };
    TimeGrid.prototype._renderColumns = function (cellsdateProfile) {
        ...
        this.colPositions = new PositionCache(this.elthis.colElstrue// horizontal
        false);
    ...
    TimeGrid.prototype.assignSegVerticals = function (segs) {
        ...
        for (i = 0; i < segs.length; i++) {
            seg = segs[i];
            applyStyle(seg.elthis.generateSegVerticalCss(seg));
        }
    };
    ...
    TimeGrid.prototype.positionToHit = function (positionLeftpositionTop) {
        ...
        if (colIndex != null && slatIndex != null) {
            ...
            var time = addDurations(this.props.dateProfile.minTimemultiplyDuration(this.snapDurationsnapIndex));
...
var TimeGridView = /** @class */ (function (_super) {
    ...
    function TimeGridView(contextviewSpecdateProfileGeneratorparentEl) {
        ...
        _this.renderHeadIntroHtml = function () {
            ...
            var dayCnt = diffDays(range.startrange.end);
            var weekText;
            if (_this.opt('weekNumbers')) {
                ...
                return '' +
                    ...
                    _this{ date: range.starttype: 'week'forceOff: dayCnt > 1 }htmlEscape(weekText) // inner HTML
                    ) +
                    '</th>';
            }
            else {
        ...
        if (!isAuto) { // should we force dimensions of the scroll container?
            ...
            if (scrollbarWidths.left || scrollbarWidths.right) { // using scrollbars?
                ...
                noScrollRowEls.forEach(function (rowEl) {
                    compensateScroll(rowElscrollbarWidths);
                });
...
var TimeGridSlicer = /** @class */ (function (_super) {
    ...
    TimeGridSlicer.prototype.sliceRange = function (rangedayRanges) {
        ...
        for (var col = 0; col < dayRanges.length; col++) {
            var segRange = intersectRanges(rangedayRanges[col]);
            if (segRange) {
                segs.push({
...
function buildDayTable(dateProfiledateProfileGenerator) {
    ...
    return new DayTable(daySeriesfalse);
}
...
export default main;
export { TimeGridView as AbstractTimeGridViewTimeGridTimeGridSlicerTimeGridView$1 as TimeGridViewbuildDayRangesbuildDayTable };
