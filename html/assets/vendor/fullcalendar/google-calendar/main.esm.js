...
import { createPluginrefinePropsrequestJsonaddDays } from '@fullcalendar/core';
...
var eventSourceDef = {
    parseMeta: function (raw) {
        ...
        if (typeof raw === 'object') {
            var standardProps = refineProps(rawSTANDARD_PROPS);
    ...
    fetch: function (argonSuccessonFailure) {
        ...
        if (!apiKey) {
            ...
            requestJson('GET'urlrequestParams_1function (bodyxhr) {
                if (body.error) {
                    onFailure({
...
function buildRequestParams(rangeapiKeyextraParamsdateEnv) {
    ...
    if (dateEnv.canComputeOffset) {
        ...
        endStr = addDays(range.end1).toISOString();
    }
